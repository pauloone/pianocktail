...
class AudioClip:
    ...
    def sample(self, start: float) -> torch.Tensor:
        ...
        if start_in_frame + self._sample_duration_in_frame > self.metadata.num_frames:
            ...
            raise ValueError(
                f"Start duration is too late: {start}, cannot extract sample"
            )
    ...
    @cached_property
    def freq_max(self):
        return self.metadata.sample_rate / 2

    @cached_property
    def n_bins(self):
        return self.freq_max / self.config.sampling.frequency_resolution
    ...
    def spectogram(self, start: float) -> torch.Tensor:
        return self.Spectogram_opj(self.sample(start))[0, :, :]
    ...
    def _frequency_to_bin(self, frequency: float) -> int:
        return frequency // self.config.sampling.frequency_resolution

    @cached_property
    def max_bin(self) -> int:
        return self._frequency_to_bin(self.config.sampling.max_frequency)

    @cached_property
    def frequency_step(self) -> int:
        return self._frequency_to_bin(self.config.sampling.frequency_step)

    @cached_property
    def _MaxPoolObj(self):
        return torch.nn.MaxPool2d((1, self.frequency_step), return_indices=True)
    ...
    def plot_spectogram(
        self, spectogram: torch.Tensor, name: str = "spectogram"
    ) -> None:
        spectogram = 20 * torch.log10(spectogram + 1e-12).numpy()
        figure, axes = pyplot.subplots()
    ...
    def write_spectogram_to_audio(self, spectogram: torch.Tensor, filename: str):
        waveform = self.InverseSpectogram_opj(spectogram)
        torchaudio.save(
            filename, waveform.reshape(1, *waveform.shape), self.metadata.sample_rate
        )
